...
function render(statusText = "") {
  ...
  panel.innerHTML = `<header class="v39-unit-create-head"><button type="button" id="v39-unit-create-back">← 管理</button><strong>ユニット作成</strong><span>軍事Lv${options.militaryLevel}</span></header>
    ...
    <footer><output id="v39-unit-create-status">${escapeHtml(statusText || (check.available ? "作成できます" : check.reasons.join(" / ")))}</output><button type="button" id="v39-unit-create-confirm" class="v39-unit-create-confirm"${check.available ? "" : " disabled"}>作成</button></footer>`;
}
