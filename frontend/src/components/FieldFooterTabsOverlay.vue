...
<section class="fs-panel">
  <nav class="fs-tabs"><button v-for="t in tabs" :key="t[0]" :class="{active:activeTab===t[0]}" @click="activeTab=t[0]">{{t[1]}}</button></nav>
  <div class="fs-body">
    <section v-if="activeTab==='squad'" class="fs-squad">
      ...
      <div v-if="squad" class="fs-split">
        ...
        <div class="fs-detail" v-if="member">
          <div class="fs-detail-head"><div><strong>{{member.name||'キャラ'}}</strong><small>{{coord(member)}}</small></div><button @click="activeTab='battle'">戦闘へ</button></div>
