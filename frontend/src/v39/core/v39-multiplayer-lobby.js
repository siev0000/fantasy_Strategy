...
function renderLobby() {
  ...
  const factionSelectionRows = playerIds.map((playerId, index) => {
    ...
    const selectedDefinition = SELECTABLE_RACES.find(race => race.key === selectedRace);
    const selectedLabel = selectedDefinition?.name || selectedRace || "未選択";
    const canEdit = roomSnapshot.phase === "lobby" && assignedId === getMyParticipantId();
    return `
      <article class="v39-faction-select-card">
        <div class="v39-faction-select-copy">
          <strong>勢力${index + 1} / ${escapeHtml(assignedParticipant?.displayName || "担当未設定")}</strong>
          <span>開始種族: ${escapeHtml(selectedLabel)}</span>
          <small>${canEdit ? "シングルと同じ種族選択画面で詳細を確認して選択します。" : "担当プレイヤーが開始種族を選択します。"}</small>
        </div>
        <button type="button" data-v39-room-action="select-faction" data-v39-room-player-id="${escapeHtml(playerId)}"${canEdit ? "" : " disabled"}>${selectedRace ? "変更" : "選択"}</button>
      </article>`;
  }).join("");
...
function openStartingFactionSelector(playerId) {
  if (!roomSnapshot || roomSnapshot.phase !== "lobby") return;
  const id = text(playerId);
  const assignedId = text(roomSnapshot.settings?.playerParticipantAssignments?.[id]);
  if (!id || assignedId !== getMyParticipantId()) return;
  const index = Math.max(0, Number(id.replace("player-", "")) - 1);
  const assignedParticipant = getParticipantById(assignedId);
  const currentRace = text(roomSnapshot.settings?.playerFactionSelections?.[id]);
  closeLobby();
  window.dispatchEvent(new CustomEvent("v39:multiplayer-race-select-request", {
    detail:{
      playerId:id,
      currentRace,
      label:`勢力${index + 1} / ${assignedParticipant?.displayName || "担当プレイヤー"}`
    }
  }));
}
...
function handleClick(event) {
  const button = event.target instanceof Element
    ? event.target.closest("[data-v39-room-action],[data-v39-room-tab]")
    : null;
  ...
  if (action === "select-faction") openStartingFactionSelector(button.dataset.v39RoomPlayerId);
...
function boot() {
  ...
  window.addEventListener("v39:multiplayer-race-selected", event => {
    const playerId = text(event.detail?.playerId);
    const raceKey = text(event.detail?.raceKey);
    if (playerId && SELECTABLE_RACE_KEYS.has(raceKey)) selectStartingFaction(playerId, raceKey);
    openLobby();
  });
  window.addEventListener("v39:multiplayer-race-selection-cancelled", () => {
    openLobby();
  });
