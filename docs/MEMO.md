# ファンタジーストラテジーゲーム 概要メモ

## コンセプト
- Civベースの国家運営ゲーム。
- 外交・資源・都市開発を主軸に進行する。
- ファンタジー要素（種族・災害・遺産）を国家運営に統合する。

## 登場種族
- 只人
- エルフ
- オーガ
- ゴブリン
- 竜人
- 悪魔
- 天使
- ヴァンパイア

## コアループ（Civベース）
- 1. 国家設定（国家体制 / 外交姿勢 / 国是 / 派閥特性）
- 2. 都市設定（通常枠の都市特性を選択）
- 3. 内政（資源生産・施設建設・社会資源管理）
- 4. 外交（交易・交換・条約）
- 5. イベント処理（時代イベント / 災害）
- 6. 次ターンへ

## 最初に決める項目
- 主要種族
- 国家体制

## 都市特性の決定ルール
- 都市特性は開始時に固定選択しない。
- 町を成長させて「都市化」するタイミングで決定する。
- 都市化時点の数値（例: 生産寄り / 交易寄り / 信仰寄り / 軍事寄り）に応じて、対応する都市特性を付与する。
- これにより、プレイヤーの運用結果が都市特性に反映される設計とする。

## 土地と開拓ルール
- 土地タイプ（例: 森 / 丘 / 平地 / 河川 / 沼地）ごとに、取得できる資源テーブルを持つ。
- 土地を確保した後、開拓方針を選んで産出資源を確定する。
- 開拓方針の例: 前線拠点化 / 資源地化 / 防衛地化 / 都市化候補地。
- 開拓により、資源産出・治安・防衛力・都市化条件の進行が変化する。

## 土地開拓の段階仕様
- 土地タイプごとに基礎産出を設定する（森=木材寄り、丘=石材/鉱物寄り、平地=食料寄り、など）。
- 各土地に開拓度（0〜100）を持たせる。
- 開拓行動で開拓度が上昇し、災害や放置で低下する。
- 開拓できる土地は「既に占拠済みの土地に隣接するマス」のみとする。
- 開拓度50以上で畑化可能。
- 開拓度80以上で町化可能（追加条件: 治安・人口が基準値以上）。
- 開拓度100で開拓完了扱いとし、物資取得率に恒久ボーナスを付与する。
- 産出計算の方針: 最終産出 = 基礎産出 × (1 + 開拓度ボーナス + 施設ボーナス)。

## 規模ごとの管理可能区画数（確定）
- 村: 2区画
- 町: 4区画
- 都市: 8区画

## 規模ごとの人口上限（実装値）
- 村: 〜159（上限160）
- 町: 160〜259（上限260）
- 都市: 260〜419（上限420）
- 大都市: 420〜（上限TBD）
- 人口は抽象値として扱い、人口1ポイント ≒ 現実100人で換算する。

## 開拓コスト方針
- 開拓は1人でも実行可能とする。
- 参加人数が少ないほど必要ターン数は増える。
- キャラLvと参加人数で開拓速度が変化する。
- 例: Lv10で4ターン、Lv5で8ターン、Lv10を4人投入で1ターン。
- 正式な計算式（Lv係数・人数係数）は後で確定する。

## 必要資源ルール
- 維持・行動で常時必要な資源は食料のみ。
- 建物建設時にのみ、対応する建設資源（木材・石材・鉱物など）を消費する。

## 採用するコア要素
- 国家体制
- 外交姿勢
- 国是
- 派閥特性
- 都市特性（通常枠・レア枠・遺産枠）
- 資源（食料 / 一般 / 戦略 / 貴重）
- 社会資源（人口 / 文化 / 信仰力 / 統治力）
- 地形
- 天候
- 災害（都市災害 / 天候災害 / モンスター災害）
- 技術分野一覧
- 施設カテゴリ
- 交易可能項目
- 市民不満度段階
- 時代イベント

## プレイヤーが設定できること
- 新規ゲーム開始時
- 種族
- 国家体制
- 外交姿勢
- 国是
- 派閥特性（任意）
- 都市化時の都市特性判定ルール（初期設定）
- ターン中
- 外交姿勢の方針転換
- 国是の再設定
- 都市運用方針の切り替え
- 交易提案（資源・条約）
- 条件達成時
- 都市特性レア枠の解放
- 都市遺産枠の建設
- 災害対策の実行

## 国データ/キャラデータ設計
- 国とキャラの保持項目、および影響先は `config/entity_data_schema.json` を基準にする。
- セーブデータ（保持）と計算ロジック（内政/外交/イベント）を分離する。

## 保留・削除（現時点）
- 補給線状態（初期MVPから除外）
- 指揮官特性（初期MVPから除外）
- クエスト・事件（災害と時代イベントに統合）
- 勝利条件ロジック（定義のみ、実装は後回し）

## タスクメモ
- 未確定ルールの管理は `docs/RULE_TASKS.md` を参照する。
- 実装順タスクは `docs/NEXT_TASKS.md` を参照する。

## これから決めること（タスク）
- 1. ゲーム開始時の設定項目を確定する
- 国家体制 / 外交姿勢 / 国是 / 派閥特性 / 種族 の必須・任意を確定
- 2. 都市の基本役割を確定する
- 生産都市 / 軍事都市 / 交易都市 / 信仰都市 の役割差を数値で確定
- 3. 施設カテゴリごとの建設条件を確定する
- 施設カテゴリごとに必要資源と解放条件を決める
- 4. 資源収支の基準式を確定する
- 食料・一般・戦略・貴重資源の毎ターン増減式を決める
- 5. 市民不満度の増減条件を確定する
- 食料収支・災害・政策で不満がどう上下するかを決める
- 6. 外交交換のルールを確定する
- 交換可能項目・交換レート・外交姿勢による補正を決める
- 7. 災害の発生ルールを確定する
- 都市災害 / 天候災害 / モンスター災害の発生率と継続ターンを決める
- 8. 時代イベントの効果を確定する
- 各イベントのメリット・デメリットと発生条件を決める
- 9. 種族ごとの基礎ステータスを確定する
- HP / ATK / DEF / MAG / RES / SPD / 基礎移動 と得意技術分野を決める
- 10. MVP範囲を固定する
- 先に実装する対象を「内政・外交・災害」の3本に限定するか確定

## UI実装メモ: 汎用モーダル（GenericModal）
- コンポーネント: `frontend/src/components/GenericModal.vue`
- 目的: データ（`fields`）を渡してフォーム型モーダルを共通化し、`modalType` で表示形式を切替える。
- 運用ルール: 簡単なモーダルは `GenericModal` を優先して使う（専用モーダルは複雑UIが必要な場合のみ）。

### 基本の使い方
- 親コンポーネントで import:
  - `import GenericModal from "./GenericModal.vue";`
- テンプレートで呼び出し:
  - `:show` で開閉
  - `title` で見出し
  - `modal-type` で表示形式
  - `:fields` でフォーム定義
  - `@field-change` で入力変更を受ける
  - `@close` で閉じる

```vue
<generic-modal
  :show="showSettingsModal"
  title="表示設定"
  modal-type="form"
  :fields="displaySettingsFields"
  :notes="displaySettingsNotes"
  @close="showSettingsModal = false"
  @field-change="applyDisplaySettingChange"
/>
```

### `modalType` の切替
- `form`: `fields` を使って入力UIを描画する。
- `form` 以外: `message` を表示する情報モードになる。

### `fields` の定義形式（form時）
- 必須:
  - `key`: 一意キー
  - `kind`: `checkbox | select | range | number | text`
  - `label`: 表示ラベル
  - `value`: 現在値
- 任意:
  - `min`, `max`, `step`, `disabled`, `help`, `suffix`
  - `options`（`select`用）: `{ label, value }[]`

### 変更イベント
- `@field-change` の payload:
  - `{ key: string, value: unknown }`
- 親側で `key` を見て state に反映する。

## UI実装メモ: スキルツリーモーダル
- コンポーネント: `frontend/src/components/SkillTreeModal.vue`
- DB: `frontend/src/data/skill-tree-db.json`
- カテゴリ一覧を渡すと、その順番で該当カテゴリだけ表示する。

### 開き方
- `openModal("skill")`
  - 既定表示: `["魔法", "軍事", "経済", "信仰"]`
- `openModal("skill", { categories: ["魔法"] })`
  - 魔法カテゴリだけ表示
- `openModal("skill", { categories: ["軍事", "信仰"] })`
  - 軍事→信仰の順で表示

### 注意
- 未登録カテゴリはスキップし、モーダル上部に `未登録カテゴリ` として表示する。

## 音声設定メモ
- 実装: `frontend/src/lib/audio-player.js`
- BGM既定: `assets/audio/bgm/古の世界地図.mp3`
- SE: `assets/audio/se` から共通再生
- 保存先: `localStorage` の `fantasy_strategy.audio_settings.v1`
  - `masterVolume`
  - `bgmVolume`
  - `seVolume`

## UIスケーリング共通ルール（固定）
- スケーリングの基準は `App.vue` の全体スケーリングのみを使う。
- 基準サイズは `frontend/src/lib/phaser-map-panel-config.js` の `GAME_VIEW_WIDTH / GAME_VIEW_HEIGHT`（現在 `1280x720`）。
- モーダル側で `transform: scale(...)` を追加しない（個別スケーリング禁止）。
- 主要レイアウトは比率指定を使わない。
  - `fr`, `minmax(...)`, `vw/vh` ベースの可変比率で組まない。
  - 固定 px 幅/高さを基本にし、必要時はコンテナをスクロールさせる。
- 画面差分は App の root scale で吸収し、各コンポーネントで再計算しない。
- 新規 UI 作成時のチェック:
  - `App.vue` 以外で追加スケーリングしていないか
  - モーダル内で比率レイアウトに戻していないか
  - 固定サイズ + overflow で収まる構成になっているか

## UI実装メモ: 資源サイドバー表示ルール（固定）
- 対象: フィールド左側の資源サイドバー（`sidebar-resource-menu` / 詳細パネル）。
- 基本方針:
  - 原則として文字ラベルを使わず、`アイコン + 数値` で表示する。
  - サイドバー行は「カテゴリアイコン + 合計値 + 増減値」を表示する。
  - 展開パネルは「資源アイコン + 保有値 + 増減値」を表示する。
- 食料カテゴリ特例:
  - `魂` と `死体` は `保有=0` かつ `増減=0` のとき非表示にする。
- アイコンサイズ調整:
  - メニュー行アイコン: `--sidebar-menu-icon-size`
  - 展開パネルアイコン: `--sidebar-detail-icon-size`
  - 以後、サイズ調整は上記CSS変数で行う（個別直書きで増やさない）。

## UI参考メモ（2026-05-03 追加 / 添付画像ベース）
- 参照画像: [ui_reference_civ_style_2026-05-03.png](./ui_reference_civ_style_2026-05-03.png)
- HUD構成の参考:
  - 上部: 資源を `アイコン + 在庫値 (+増減)` で横並び常時表示。
  - 左側: 行動ボタンを縦スタック（`Sleep / Pillage / Explore / Wait` のような即押し導線）。
  - 右側: 通知ログを縦積みオーバーレイで表示（マップを押し出さない）。
  - 下部: 戦闘時は「攻撃側/防御側の要約 + 実行ボタン」を1枚のカードに集約。
  - 右下: ミニマップを固定表示して視点移動の導線を残す。
- 見た目方針:
  - 半透明パネル + 高コントラスト数値で、地形の視認を優先する。
  - 原則「文字よりアイコン優先」、必要最小限の文言のみ表示する。

## マップエフェクト外部再生API（2026-05-06）
- 実装先: `frontend/src/components/PhaserMapGeneratorPanel.vue`
- 目的: 攻撃処理など別処理から、`向き / 角度 / 再生先` を指定してマップ上エフェクトを再生する。

### 呼び出し方法1: windowブリッジ
- `window.__fantasy_strategy_map_effect_bridge__.play(payload)`
- イベント名は `window.__fantasy_strategy_map_effect_bridge__.eventName` に保持。

### 呼び出し方法2: CustomEvent
- `window.dispatchEvent(new CustomEvent("fantasy-strategy:play-map-effect", { detail: payload }))`

### payload（主な指定）
- 再生対象:
  - `src`（直接画像URL）
  - または `effectName` / `name`（`assets/effect/320×240/*.webp` のファイル名）
- 角度:
  - `angleDeg`（0〜360、優先）
  - `directionIndex`（0:E, 1:NE, 2:NW, 3:W, 4:SW, 5:SE）
  - `direction`（`"E" | "NE" | "NW" | "W" | "SW" | "SE"`）
- 再生先:
  - `worldX`, `worldY`（ワールド座標を直接指定）
  - または `tileX`, `tileY` / `tileCoord` / `tileKey`（タイル座標指定）
- 任意:
  - `scalePercent`（10〜400）
  - `tint`（色変更。`0xff0000` / `"#ff0000"` 形式）

```js
window.__fantasy_strategy_map_effect_bridge__.play({
  effectName: "炎全体",
  tileX: 22,
  tileY: 14,
  directionIndex: 1,
  scalePercent: 60
});
```

## 装備運用メモ（2026-05-10）
- 装備変更UIの主対象は `ネームド/ヒーロー系ユニット` とする。
- `モブ` は個別装備を直接編集しない運用にする。
- モブの装備は固定プリセット参照のみ（クラスに紐づく既定構成を使う）。
- アイテム一覧モーダルには、モブ向け装備は表示しない。
- モブ装備で許可する更新は `レア度一新` のみ。
- レア度一新時は、サイド作成時と同じ素材ルールで資材を消費する。

## UI共通ルール: 情報重複と折りたたみ（固定 / 2026-09-14）
- ヘッダー、タブ、選択状態などを見れば判断できる情報は、本文へ重複表示しない。
  - 例: `王都` タブが選択中なら `選択中: 王都` を本文へ表示しない。
  - `拠点`、`土地`、`部隊` なども、画面上の選択状態から分かる名称・種別を説明文として繰り返さない。
- UI上の情報量は必要最小限とし、同じ意味の見出し・ラベル・説明文を重ねない。
- 項目数が多く縦に長くなる画面では、項目単位の折りたたみ表示を基本とする。
  - 閉じている状態: `▷ 項目名`
  - 開いている状態: `▽ 項目名`
  - 開いている場合だけ内訳・詳細を表示する。
- 折りたたみは項目ヘッダー自体を押して開閉する。
- `開く` / `閉じる` のような別文字ボタンは原則追加しない。
- 閉じている項目の内部内容は完全に非表示にする。
- 特にスマホ縦画面や下段パネルなど、縦方向の表示領域が限られるUIではこのルールを優先する。
- アイコンのみを常時表示するUIでは、名称や種類の文字を常時併記しない。
- アイコンの名称・種類などの文字情報は、タップ・クリック・選択時のポップアップまたは詳細表示で確認できるようにする。
- PCのホバー表示は補助として使用してよいが、ホバーできないスマホでも同じ情報へ到達できるよう、タップ・選択を基本操作とする。
- マップ上のユニット・施設・地点なども同様に、通常時はアイコン中心、必要な文字情報は選択時に表示する。
- ポップアップは原則同時に1つだけ表示する。別のアイコンを選択した場合は現在のポップアップを閉じ、新しい対象へ差し替える。
- 同じアイコンを再度タップ・クリックした場合、またはポップアップ外をタップ・クリックした場合は閉じる。
- 折りたたみ項目は複数を同時に開ける。1項目を開いたことを理由に他項目を自動で閉じない。
- `破壊`、`解体`、`放棄`、`全軍解散` など、取り返しのつかない危険操作だけ確認を表示する。通常操作には不要な確認を挟まない。
- 拠点名など横並びタブが多い場合は、文字やタブ幅を縮小して詰め込まず横スクロールで表示する。
- 選択状態は `選択中` などの文字を追加せず、枠・背景・下線などの視覚表現で判別できるようにする。